...
int main(void){
	...
	// Default to low-pass filters
	Audio_SetFilters(&ctx, FILTER_LOW_PASS, FILTER_LOW_PASS, FILTER_LOW_PASS);
	
	mcp4011_init_all();  // Initialize all potentiometers
	mcp4011_set_all(0);  // Start with all pots at position 0

	SharedBuffer *my_buffer = nullptr; // Pointer to shared memory buffer
